...
import de.herk.kfza.data.model.SourceType
...
class SpecialPlateAssetLoader(private val context: Context) {
    ...
    companion object {
        const val ASSET_PATH = "data/special_identifiers.json"
...
private class SpecialPlateJsonReader(private val source: String) {
    ...
    private fun readEntry(): PlateEntry {
        ...
        return PlateEntry(
            ...
            region = values.requiredString("region"),
            status = values.requiredString("status").let { de.herk.kfza.data.model.PlateStatus.valueOf(it) },
